---
title: "Task 1 - Retail Strategy and Analytics - Quantum Virtual Internship"
...
```
_dates are in the right format_
...
_As we are only interested in words that will tell us if the product is chips or
not, let's remove all words with digits and special characters such as '&' from our
set of product words. We can do this using `grepl()`._
```{r}
...
```
 ...
 We saw that we have _whitespace_ maximum number of times and the second most occuring word is _chips_
...
#### Pack size
A new column _PACK SIZE_ added to the data frame _transaction_data_
